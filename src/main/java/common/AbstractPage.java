...
public abstract class AbstractPage {
    ...
    public void clickItemInListItems(WebDriver webDriver, String expected){
        List<WebElement> allItems = finds(webDriver,"//ul[@id='ui-id-1']//li//a//span");
        for (WebElement item : allItems){
            if (item.getText().equals(expected)){
                item.click();
            }else {
                try {
                    throw new Exception("------ Expected khong tom tai -------");
                } catch (Exception e) {
